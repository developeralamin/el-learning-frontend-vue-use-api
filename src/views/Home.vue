                    ...
                    <div class="card-body">
                        <table class="table">
                            ...
                            <tbody v-if="lessons.length">
                                <tr v-for="product in lessons" :key="product.id">
...
export default {
    ...
    methods: {
          loadLesson(){
               axios.get('http://127.0.0.1:8000/api/lesson')
            .then(response  => {
              this.lessons = response.data.alamin
            })
        },

        //  async deleteProduct(product){
        //     await axios.delete(`http://127.0.0.1:8000/api/lesson/${product.id}`).then(() => {
